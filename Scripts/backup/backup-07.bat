...
REM Escolha a Unidade
	...
	echo # - - - - - - - - - - - - - - - - - - - - - - - - - #
	echo.
		set /p disco-d="Unidade do Disco D: " 
		echo %disco-d%
...
REM Disco_C
	...
	echo.
		CHCP 1252  >NUL
			start FastCopy\%fastcopy%  /cmd=diff /exclude="System Volume Information\; Config.Msi\; inetpub\; Intel\; Recovery\; PerfLogs\; ProgramData\; Windows\; Users\; *.sys; MSOCache\; $Recycle.Bin\; Windows\; Users\; *.sys; MSOCache\; $Recycle.Bin\; System Volume Information\; $WINDOWS.~BT\; autoexec.bat; Qoobox\; System Volume Information\; Config.Msi\; inetpub\; Intel\; Recovery\; PerfLogs\; *.sys; MSOCache\; $Recycle.Bin\;" /logfile=FastCopy\Log\disco-d.log /speed=512 /bufsize=256 /error_stop=FALSE /force_start /skip_empty_dir=FALSE /acl=FALSE /verify=FALSE /estimate %disco-d%:\ /to="backup\disco-d"
